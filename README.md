...
├── monitor-key.pem           # SSH private key (generated by Terraform)
└── terraform/
    ├── main.tf               # VPC, subnet, IGW, route table, SG, EC2, SSH key, Lambda scheduler
...
```bash
ssh -i marukyu/monitor-key.pem ubuntu@$(cd marukyu/terraform && terraform output -raw instance_public_ip)
```
